...
function CollaborativeEditor() {
  ...
  const { post, tabId } = useBroadcastChannel<CollabEvents>(
    'collab-editor',
    {
      ...
      textEdit: ({ userId, range, text }) => {
        ...
        setContent(prev =>
          prev.slice(0, range.start) + text + prev.slice(range.end)
        )
      },
  ...
  return (
    <div>
      <textarea
        value={content}
        onChange={(e) => handleTextChange(e.target.value, 0, e.target.value.length)}
      />
...
- [useLocalStorage](/api/hooks/useLocalStorage) - 영구 저장 + 탭 간 동기화
- [useSessionStorage](/api/hooks/useSessionStorage) - 세션 저장 + 같은 탭 내 동기화
