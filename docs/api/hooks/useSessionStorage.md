...
function MultiStepWizard() {
    ...
    const [wizardData, setWizardData] = useSessionStorage<Partial<WizardData>>(
        'wizardData',
        {}
    );
...
function Editor() {
    const [settings, setSettings] = useSessionStorage<TabSettings>(
        'editorSettings',
        {
            fontSize: 14,
            compactView: false,
    ...
    return (
        <div>
            <p className="info">
                각 탭마다 독립적인 설정을 가집니다! 새 탭을 열면 기본 설정으로
                시작합니다.
            </p>
            <label>
                글자 크기:
