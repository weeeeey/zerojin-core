...
function TrackingButton() {
    const trackClick = useDebouncedCallback(
        () => {
            console.log('사용자 클릭');
...
```tsx
// 특정 타입을 가진 함수
const calculate = useDebouncedCallback((a: number, b: number): number => {
    return a + b;
}, 300);
...
const updateUser = useDebouncedCallback(
...
다음과 같은 경우 `useDebouncedCallback`를 사용하세요:
...
| 시나리오    | useDebouncedCallback       | useThrottle            |
| ----------- | -------------------------- | ---------------------- |
